...
namespace webrtc {
    namespace {
        ...
        constexpr size_t kMaxSamplesPerChannel = AudioBuffer2::kMaxSampleRate / 100;
    ...
    AudioBuffer2::AudioBuffer2(size_t input_rate,
                             size_t input_num_channels,
                             size_t buffer_rate,
                             size_t buffer_num_channels,
                             size_t output_rate,
                             size_t output_num_channels)
            : AudioBuffer2(static_cast<int>(input_rate) / 100,
                          input_num_channels,
...
// remainder is zero.
    ...
    AudioBuffer2::AudioBuffer2(size_t input_num_frames,
                             size_t input_num_channels,
    ...
    AudioBuffer2::~AudioBuffer2() = default;

    void AudioBuffer2::set_downmixing_to_specific_channel(size_t channel) {
    ...
    void AudioBuffer2::set_downmixing_by_averaging() {
        downmix_by_averaging_ = true;
    }

    void AudioBuffer2::CopyFrom(const float *const *stacked_data,
    ...
    void AudioBuffer2::CopyTo(const StreamConfig &stream_config,
    ...
    void AudioBuffer2::CopyTo(AudioBuffer2 *buffer) const {
    ...
    void AudioBuffer2::RestoreNumChannels() {
    ...
    void AudioBuffer2::set_num_channels(size_t num_channels) {
...
// The resampler is only for supporting 48kHz to 16kHz in the reverse stream.
    void AudioBuffer2::CopyFrom(const int16_t *const interleaved_data,
                               const StreamConfig &stream_config) {
        RTC_DCHECK_EQ(stream_config.num_channels(), input_num_channels_);
        RTC_DCHECK_EQ(stream_config.num_frames(), input_num_frames_);
        RestoreNumChannels();
    ...
    void AudioBuffer2::SplitIntoFrequencyBands() {
        splitting_filter_->Analysis(data_.get(), split_data_.get());
    }

    void AudioBuffer2::MergeFrequencyBands() {
        splitting_filter_->Synthesis(split_data_.get(), data_.get());
    }

    void AudioBuffer2::ExportSplitChannelData(
            size_t channel,
    ...
    void AudioBuffer2::ImportSplitChannelData(
            size_t channel,
