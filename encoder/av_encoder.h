...
class AvEncoder : public VideoEncoder {
public:
	struct LayerConfig 
	{
		int simulcast_idx = 0;
...
public:
	explicit AvEncoder(const cricket::VideoCodec& codec);
	~AvEncoder() override;

    webrtc::VideoCodec GetCodecSettings();

	int32_t InitEncode(const webrtc::VideoCodec* inst, const VideoEncoder::Settings& settings) override;
...
private:
	...
	bool EncodeFrame(int index, const VideoFrame& input_frame, char *data, int *size);
